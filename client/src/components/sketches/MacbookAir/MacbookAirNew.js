...
class MacbookAir extends React.Component {
  ...
  componentWillUnmount() {
    window.removeEventListener('load', this.getDimensions);
  ...
  getDimensions() {
    ...
    var {windowDim, sliderDim} = this.state;
    ...
    // console.log("START", windowDim);
    if (device === "desktop") getPortraitDimensions();
    // else if (availableHeight >= originalDim.h) return getPortraitDimensions();
    else if (windowAspectRatio < 1) getPortraitDimensions();
    else getLandscapeDimensions();

    // console.log("END", windowDim);
    this.setState({windowDim, sliderDim})

    function getLandscapeDimensions() {
      windowHeight-=toolbarH;
      windowDim.w = originalDim.w;
      windowDim.h = originalDim.h;
      windowDim.x = 0;
      windowDim.y = 0;
      sliderDim.w = 80;
    ...
    if (this.props.dimensions.windowWidth <= 0 || windowDim.w <= 0) {
      ...
      return (
        <Frame title="macbook air" content={
          ...
          width={windowDim.w+2} height={windowDim.h} x={windowDim.x+100} y={windowDim.y} 
          />
      )
    }
  }
  ...
  render() {
    ...
    // const {windowDim, sliderDim} = this.state; //frameDimensions[0];
